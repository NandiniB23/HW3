	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
						(currentChar >= 'a' && currentChar <= 'z' )) 	// Check for a-z
						 {												// Updated to only allow alphabetical characters for the first space for user name 
					nextState = 1;
			...
			case 1: 
				...
				// Updated to allow special characters '.' '-' '_' in user name in between alphanumeric characters
					nextState = 2;
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z, and/or 0-9 .\n";
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
